...
const whyMatters = [
  "Accounts for site and building\nenvelope factors",
  "Supports early-phase\nplanning",
  "Helps manage external cost\nrisks",
]
...
export function ExteriorEstimatingContent() {
  return (
    <section className="bg-black text-white">
      <section className="mx-auto max-w-7xl px-6 pb-16 pt-8">
        <nav className="mb-16 text-sm text-[#d9d9d9]/60">
          ...
          <span>Exterior Estimation</span>
        </nav>

        <div className="grid grid-cols-1 items-center gap-12 lg:grid-cols-2">
          <div className="max-w-xl">
            <h1 className="mb-8 text-4xl font-bold leading-tight md:text-5xl">
              <span className="text-[#C9A86C]">Planning</span> Costs Beyond the Building Envelope
            </h1>

            <p className="text-justify text-sm leading-relaxed text-[#d9d9d9]/75 md:text-base">
              Exterior estimating services focus on site conditions, building
              protection, and external systems that directly impact project
              timelines and budgets. These estimates help teams anticipate
              external factors early.
            </p>
          </div>

          <div className="flex justify-center lg:justify-end">
            <ExteriorHexIcon
              id="exterior-hero-icon"
              topColor="#8A6AB8"
              layerColor="#5A4080"
            />
          </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16">
        <h2 className="mb-8 text-xl font-bold md:text-2xl">Why Exterior Estimates Matter:</h2>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16">
        <h2 className="mb-6 text-center text-3xl font-bold md:text-4xl">
          Types of Exterior Estimates
        </h2>
        <p className="mx-auto mb-12 max-w-5xl text-center text-sm leading-relaxed text-[#d9d9d9]/75 md:text-base">
          Exterior estimating addresses all site and envelope components critical
          to completing the building. These key areas allow precise cost control
          and planning for the project&apos;s outer structure.
        </p>

        <div className="mx-auto max-w-5xl space-y-8">
          {exteriorTypes.map((item, index) => (
            <div
              key={item.label}
